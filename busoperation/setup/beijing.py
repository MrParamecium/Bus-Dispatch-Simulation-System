...
from .beijing_57_data.dataloader import DataLoader
...
class BJ_route_57_Network(Network):
    ...
    def _define_network(self):
        ...
        for head_node, tail_node in zip(node_ids[:-1], node_ids[1:]):
            ...
            tt_mean = link_time_info[tail_node]['loc']
...
class BJ_route_57_Route_Schema(Route_Schema):
    def __init__(self) -> None:
        super().__init__()
    ...
    def _define_od_table(self) -> Dict[str, Dict[str, Dict[str, float]]]:
        ...
        upper_triangle = pd.DataFrame(
            ...
            index=od.index.astype(str),
            columns=od.columns
        )
        stacked = upper_triangle.stack()  # 将矩阵压平为多级索引序列

        od_rate_table = defaultdict(dict)
        for (origin, dest), value in stacked.items():
            if value != 0:  # 仅保留非零值
                od_rate_table[origin][dest] = value/60
