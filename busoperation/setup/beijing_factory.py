...
route_57_rtd_time = [
    90, 292, 466, 547, 673, 835, 991,
    1173, 1554, 1719, 1861, 2074, 2243,
    2385, 2563, 2694, 2780, 2947, 3027,
    3075, 3180, 3245, 3387, 3546, 3623,
    3692, 3836, 3944, 4267, 4356, 4511,
    4668, 4781, 5106
]

route_57_stop_rtd_time = defaultdict(dict)
for i, t in enumerate(route_57_rtd_time, start=1):
    route_57_stop_rtd_time['4'][str(i)] = t-50

class Bj_Route57_Components_Factory:
    ...
    def create_virtual_bus(self, blueprint: Blueprint, agent: Agent) -> VirtualBus:
        ''' Create a virtual bus with data

        '''
        virtual_bus = VirtualBus(blueprint)
        virtual_bus.initialize_by_data(route_57_stop_rtd_time)
        print('created virtual bus with data')
    ...
    def create_links(self, blueprint: Blueprint, Agent) -> Dict[str, Link]:
        links = {}
        for link_id, link_geometry in blueprint.network.link_geometry_info.items():
            ...
            link = DistributionLink(link_id, link_geometry, link_distribution,Agent)
            links[link_id] = link
        return links
