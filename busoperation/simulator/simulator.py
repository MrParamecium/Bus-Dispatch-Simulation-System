...
class Simulator:
    ...
    def step(self, t: int, stop_bus_hold_time: Dict[Tuple[str, str, str], float]) -> Snapshot:
        ...
        for bus in self._total_buses:
            #bus.total_pax_in_vehicle_time = np.sum([t- pax.board_time for pax in bus._paxs])#+bus.leaving_pax_in_vehicle_time
            if bus.status != 'finished':
                bus.accumulate_in_vehicle_delay()

            #print(t,bus.bus_id,len(bus._paxs),np.sum([pax.in_vehicle_delay for pax in bus._paxs]))
    ...
    def take_snapshot(self, t: int) -> Snapshot:
        ...
        snapshot = self._tracer.take_snapshot(
            t, self._links, self._stops, self._holder) #增加了link的snapshot
        return snapshot
