...
class StopLog:
    ...
    def record_when_bus_rtd(self, route_id: str, bus_id: str, speed: float,t: int, **kwargs) -> None:
...
class BusRunningLog:
    def __init__(self, schedule_headway: float, virtual_bus_stop_arrival_time: Dict[str, float],
        ...
        # record the speed at each link
        self.stop_link_speed: Dict[str, int] = {}
    ...
    def record_when_enter_link(self, link_id: str, tt_deviation: float,speed: float) -> None:
        self.link_tt_deviation[link_id] = tt_deviation
        self.stop_link_speed[link_id] = speed
...
class LinkLog:
    ...
    按不同路线分类存储公交车进入当前链接的时间、车辆ID和进入时的速度信息

    Attributes:
        link_id: 当前链接的唯一标识符
        route_enter_time_seq: 记录各路线公交车进入链接的时间序列，键为路线ID，值为时间列表
        route_enter_bus_id_seq: 记录各路线进入链接的公交车ID序列，键为路线ID，值为车辆ID列表
        route_enter_speed_seq: 记录各路线公交车进入链接时的速度序列，键为路线ID，值为速度列表
    """
    def __init__(self, link_id: str) -> None:
        self.link_id = link_id
        # route -> [arrival time at this link]
        self.route_enter_time_seq: Dict[str, List[str,float]] = defaultdict(dict)
        # route -> [arrival bus_id]
        self.route_enter_bus_id_seq: Dict[str, List[str]] = defaultdict(list)
        # route -> [speed when enter the link]
        self.route_enter_speed_seq: Dict[str, List[str,float]] = defaultdict(dict)

    def record_when_bus_enter(self, route_id: str, bus_id: str, speed:float,t: int) -> None:

        self.route_enter_time_seq[route_id][bus_id]=t
        self.route_enter_speed_seq[route_id][bus_id]=speed
        self.route_enter_bus_id_seq[route_id].append(bus_id)
