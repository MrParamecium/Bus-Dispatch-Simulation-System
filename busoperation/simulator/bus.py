...
from .trajectory import TrajectoryPoint,InfoPoint
import numpy as np
...
class Bus:
    ...
    def __init__(self, bus_id: str,
        ...
        self.total_pax_in_vehicle_time: int = 0
        self.total_pax_out_vehicle_time: int = 0
        self.total_pax_num: int = 0
        self._info: Dict[int, InfoPoint] = {}
        self.leaving_pax_in_vehicle_time: int = 0
        self._last_deviation: int = 0
    ...
    def board(self, pax: Pax, t: int) -> None:
        ...
        self.total_pax_out_vehicle_time += pax.out_vehicle_delay
        self.total_pax_num  += 1
    ...
    def update_location(self, t: int,
        ...
        self._trajectory[t] = TrajectoryPoint(
            spot_type, spot_id, self.loc_relative_to_terminal, self.speed,status)
        self.total_pax_in_vehicle_time= np.sum([pax.in_vehicle_delay for pax in self._paxs])+self.leaving_pax_in_vehicle_time
        self._info[t]=InfoPoint(spot_type, spot_id, self.loc_relative_to_terminal, self.speed,status,len(self._paxs),self.total_pax_num,self.total_pax_out_vehicle_time, self.total_pax_in_vehicle_time,self.leaving_pax_in_vehicle_time)
    ...
    def take_snapshot(self) -> BusSnapshot:
        ...
        bus_snapshot = BusSnapshot(self._bus_id, self._route_id, self._is_need_to_hold, pax_num,self.loc_relative_to_terminal,
                                   ...
                                   self.bus_log.stop_epsilon_departure, self.bus_log.visited_stops,self.bus_log.stop_link_speed)
        return bus_snapshot

    def get_remaining_capacity(self):
        return self._capacity - len(self._paxs)
